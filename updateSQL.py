...
def main(file_name, where, colindex):
    # colindex为需要插入的列
    tables = excel_table_bycol(file_name, where, colindex, table_name=u'新增data')
    t_name = tables.pop(0).decode()
    #key_list = ','.join(tables.pop(0)).encode('utf8').decode()   #list转为str
    #print('t_name:', t_name)
    #print('key_list:', key_list)
    #print('tables:', tables)
    # sql_line = "UPDATE prod_rate_config SET (PRODCODE) = (SELECT PRODCODE FROM prod_rate_config WHERE serialno = '2017070400000001') WHERE serialno = '2018010100000001';"
